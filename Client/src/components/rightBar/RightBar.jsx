...
const handleUnfollow =async(e)=>{
  ...
  if(rel.includes(userID))
  {
    ...
    try{setRel(rel.filter(item => item !== del.data[0]))}
    catch(e){setRel([])};
  }
}
          ...
          {users.map((user)=>(
            ...
            <div className="user">
              ...
              <div className="buttons">
                <button onClick={handleFollow} value={user._id}>follow</button>
                <button onClick={handleUnfollow} value={user._id}>dismiss</button>
            </div>
          </div>
          ))}
        </div>
